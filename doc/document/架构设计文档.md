...
flowchart TB
    ...
    subgraph GM["game-gm-server（HTTP-only：入口下发 + GM 后台）"]
        GMHTTP["HTTP :80<br/>GmStart 路由（/gateway、/gm/*）"]
    end
...
- **game-common**：
  ...
  - 扩展点：`BaseMessageAction`（模板方法 `LogRequest → doAction → LogResponse`）、注解 `@MessageClassMapping`（类级，声明处理的 msgId 与 isNet）、`@MessageMethodMapping`（方法级，可声明多个 msgId）。
...
| msgId | 4 字节 int | 协议号，见 `doc/proto3/rpc.proto` 的 `RpcNameEnum` |
...
flowchart TB
    ...
    C --> D["msgId → Action Class<br/>存入 MessageManager.actorToHandleActionMap"]
    D --> E{"isNet == true ?"}
    B -->|否| E
    E -->|是| F["msgId → self()<br/>存入 msgIdToHandleActorMap"]
    E -->|否| G["仅本 Actor 内部可处理"]
    A --> H["反射扫描 @MessageMethodMapping 方法"]
    H --> I["msgId → 方法名 存入 mappingMethodMap"]
    I --> E
```
...
- 同一 msgId 被重复注册会打 error 日志（首个注册生效）。
...
flowchart TB
    ...
    T -->|NetMessage / RemoteMessage / LocalMessage| R{"msgId 查 Action 类?"}
...
逻辑服的顶层分发：`LogicServerActor` 收到 `NetMessage` 后调 `MessageManager.handleRequest()`，按 `msgIdToHandleActorMap` 把消息投递给对应的业务 Actor（LoginActor / RegistActor / MatchActor），再由 6.2 的机制落到 Action。
...
flowchart TB
    subgraph gwsys["ActorSystem: gateway_N"]
        GWA["gatewayActor (GatewayNodeActor)<br/>顶号强踢"]
        CAA["ClientSessionActor ×连接数<br/>(用户树)"]
        RAA["GatewayResponseActor ×连接数<br/>(ClientSessionActor 子节点)"]
        CAA --- RAA
    end

    subgraph lg["ActorSystem: logicServer_N"]
        LSA["logicServerActor<br/>分发/离线通知"]
        LA["loginActor"]
        RA2["registActor (仅主)"]
...
```mermaid
flowchart TB
    A["1. 启动 Nacos / MySQL / Redis / Mongo"] --> B["2. 启动 gm-server（HTTP-only）"]
    B --> C["3. 启动 logic-server(含主) / gateway / battle / chat"]
    C --> D["4. CommonStart：Log4j2 → Nacos 注册(含 metadata) → ActorSystem → Redis → Mongo"]
    D --> E["5. XxxStart.startBusiness 创建业务 actors + Netty/HTTP"]
    E --> F["6. 节点间通信按 NodeKind+nodeId 从 Nacos 目录取 ActorRef"]
```
...
| 内容 | 位置 |
|---|---|
| 消息载体与协议头 | `game-common/.../msg/NetMessage.kt`、`RemoteMessage.kt` |
| RPC 统一入口 | `game-common/.../framework/akka/RemoteCall.kt`（NodeKind+nodeId → tell/askAwait） |
| 分发基类 | `game-common/.../akka/BaseMessageActor.kt` |
| 网关编解码 | `game-gateway-server/.../network/ProtocolDecoder.kt` 等 |
| 网关路由 | `game-gateway-server/.../actor/ClientSessionActor.kt` + `service/GatewayZone.kt` |
| 节点发现 | `game-common/.../framework/net/NacosService.kt`（Nacos naming，无 GM 注册） |
| 登录/匹配 | `game-logic-server/.../actor/LoginActor.kt`、`service/MatchService.kt` |
| 战场与事件 | `game-battle-server/.../BattleRooms.kt` |
| 聊天 | `game-chat-server/.../ChatActors.kt` |
