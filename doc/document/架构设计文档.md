...
| 协议号 | 名称 | 方向 |
|---|---|---|
| 400101 | `RemoteRpcRegistServer` | 各业务服 → GM（注册），GM → 各服（响应） |
...
| 300001 | `LocalRpcRegistToGmServer` | 各服进程内：触发向 GM 注册（定时重试） |
...
功能服务器（gateway/logic/battle/chat）启动后都会执行同一套注册流程，以网关为例：

```mermaid
sequenceDiagram
    participant GW as GatewayActor
    participant GM as GmActor(gm-server)
    participant R as Redis

    Note over GW: preStart() 启动定时任务
    loop 每 5 秒直到注册成功
        GW->>GM: RemoteRpcRegistServer<br/>(RegistServerRequest: type/id/akkaPath/connectPath)
    end
    GM->>R: 写入 serverId → akkaPath 映射<br/>(logic 另写 mainLogicServerId)
    GM->>GM: getContext().watch(sender())
    GM-->>GW: RegistServerResponse(Ok)
    GW->>R: 上报自身负载 setOneGatewayLoadBalance
    Note over GW: 取消重连定时任务<br/>isConnectToGm = true
```

关键实现：

- `GmActor.preStart()`：GM 启动时**清空 Redis 中全部注册/负载数据**，等待各服务器重新注册（保证注册表与存活节点一致）。
- `GmActor.registServer()`（`@MessageMethodMapping(RemoteRpcRegistServer_VALUE)`）：按 `RemoteServerTypeEnum` 分四类处理；主逻辑服具有唯一性（重复注册返回 `RemoteRpcRegistServerErrorHasRegisted`）。
- 断线感知：GM `watch` 了每个注册方的 sender，收到 `Terminated` 后调 `removeXxxServer()` 删除 Redis 注册信息（主逻辑服下线会把 `mainLogicServerId` 置 0）。
- 反向容错：功能服 `doTerminated()` 检测到 GM 断开后，重新开启 5 秒注册定时任务，直到 GM 恢复。
...
```mermaid
flowchart LR
    CH["Netty Channel"] -->|"channelActive<br/>attachChannelActor()"| CA["ChannelActor<br/>（每连接一个，线程隔离）"]
    CA -->|"preStart() 创建"| RA["ResponseActor<br/>（每连接一个）"]
    CA -->|"登录类响应包装"| NRM["NetResponseMessage"]
    RA -->|"回包直接 writeAndFlush"| CH
    OC["OnlineClientManager<br/>sessionId → channel<br/>sessionId → logicServerId 等映射"] --- CA
```

- 每个 Channel 绑定一个 `ChannelActor`（存于 Channel attr `nettyChannelToActorKey`）；SessionId 由 Redis 自增键 `sessionIdAutoIncrease` 分配，随 Channel 存 attr。
- `ChannelActor` 内嵌 `userId`：**登录成功后才绑定**，作为后续请求的鉴权依据——未登录连接发匹配/对战/聊天协议会被直接 `channel.close()`。
- 每个连接还有一个附属 `ResponseActor`：后端业务服回包时以它为 sender，它与请求消息（同为 NetMessage）不会在 ChannelActor 中混淆；登录响应经 `NetResponseMessage` 通知父 Actor 绑定 userId，其余响应直接写回客户端。
