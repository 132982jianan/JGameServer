...
object MessageRouterService {
    ...
    suspend fun onLoginResponse(msg: NetMessage) {
        when (msg.errorCode) {
            Rpc.RpcErrorCodeEnum.Ok_VALUE -> {
                ...
                SessionService.userInfo = userInfo
                ...
                when (actionState.number) {
                    ...
                    CommonEnum.UserActionStateEnum.Matching_VALUE -> {
                        swing {
                            ViewManagerService.hallFrame = HallFrame(userInfo).also { it.isVisible = true }
                            ViewManagerService.loginFrame?.dispose()
                            ViewManagerService.loginFrame = null
                        }
                    }
    ...
    suspend fun onMatchResponse(msg: NetMessage) {
        when (msg.errorCode) {
            Rpc.RpcErrorCodeEnum.Ok_VALUE -> {
                ...
                swing {
                    ViewManagerService.hallFrame?.matchBtn?.isEnabled = false
                    ViewManagerService.hallFrame?.unmatchBtn?.isEnabled = true
                }
            }
    ...
    suspend fun onEventMsgListPush(msg: NetMessage) {
        ...
        BattleEventService.doEvent(push.eventMsgList.msgListList)
    }
    ...
    suspend fun onPlacePiecesResponse(msg: NetMessage) {
        when (msg.errorCode) {
            Rpc.RpcErrorCodeEnum.Ok_VALUE -> {
                ...
                BattleEventService.doEvent(resp.eventList.msgListList)
            }
    ...
    suspend fun onGetBattleInfoResponse(msg: NetMessage) {
        when (msg.errorCode) {
            Rpc.RpcErrorCodeEnum.Ok_VALUE -> {
                ...
                swing {
                    ViewManagerService.hallFrame?.dispose()
                    ViewManagerService.loginFrame?.dispose()
                    ViewManagerService.hallFrame = null
                    ViewManagerService.battleFrame = BattleFrame(battleInfo).also { it.isVisible = true }
                }
                // 发送加入对战聊天室请求
                ...
                NetService.send(NetMessage(Rpc.RpcNameEnum.JoinChatRoom_VALUE, builder))
    ...
    suspend fun onBattleChatTextPush(msg: NetMessage) {
        ...
        val senderName = ViewManagerService.battleFrame?.opponentUserInfo?.nickname ?: "Player"
        swing {
            ViewManagerService.battleFrame?.appendChat("$senderName: ${push.text}\n")
        }
    }
